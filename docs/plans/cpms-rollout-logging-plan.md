...
```
   ...
   Then, after `machineMgr := openshift.NewMachineManager(...)`, hoist
   `targetFDNames := failureDomainNames(migration.Spec.FailureDomains)` (it is currently
   computed inside Step 3 — remove that local; keep the variable name), and insert the
   new gate **after the existing worker and Node readiness validation** (the
   `checkWorkerReadiness` call and its `!readiness.Ready` requeue), so the rollout and
   scale-down path runs only when the target workers are ready and the CPMS already
   targets the target failure domains. At that point `allTargetMSExist` is implied
   (the Step-1 loop and the readiness check both ran), so the gate tests only
   `cpmsUpdated`:

```go
	// Once target workers are ready and the CPMS already targets the target
	// failure domains, the CPMS update step is done: continue from the rollout
	// and scale-down path, derived entirely from cluster state.
...
```
   ...
   `err` is already in scope; `:=` is valid because `cpmsUpdated` is new.
   The `strings` and `apimeta` imports remain (used elsewhere in the file).
   Existing error handling of `checkWorkerReadiness` is unchanged.
...
7. **Doc comment** for `ensureWorkloadMigratedRolloutAndScaleDown`: remove the
   sentence "Call when condition message indicates we are past \"CPMS updated\" (e.g.
   \"Control plane rollout\" or \"Old workers\" or we have observed generation and rollout
   complete)." — the gate is cluster-state derived: the function is called only once
   target workers are ready and the CPMS already targets the target failure domains.
...
objects:     [newInfrastructureForRollout(), newCPMSForRollout(true, false)]
...
4. New test `TestEnsureWorkloadMigratedRolloutGate` — table-driven, calls
   ...
   loop (fakekube, configfake with the infra object, fakemachineclient with the rest,
   `record.NewFakeRecorder(20)`; objects are split into the three fake clients by
   `splitRolloutTestObjects`). Migration fixture (built inline per case) — note the
   topology type is `configv1.VSpherePlatformTopology` (the type used by
   `VSpherePlatformFailureDomainSpec.Topology`), with `ComputeCluster` set:
...
```
   ...
   Fixtures used: `newTargetMachineSetForRollout(name, server, replicas)` (target-side
   MachineSet), `newSourceMachineSetForRollout(replicas)` (source MachineSet named
   `source-worker-a` on `source.example.com`), `newReadyWorkerMachineForRollout(
   machineSetName, nodeName)` (Running machine with NodeRef), `newReadyNodeForRollout(
   name)` (Ready node). Cases:
   - "routes to rollout path when CPMS targets failure domains and workers are ready":
     objects = infra, `newTargetMachineSetForRollout(targetMSName, "target.example.com", 1)`,
     `newReadyWorkerMachineForRollout(targetMSName, "worker-node-1")`,
     `newReadyNodeForRollout("worker-node-1")` (readiness gate satisfied),
     `newCPMSUpdatedForRollout([]string{"target-fd-1"}, false)`,
     `newSourceMachineSetForRollout(1)`. Expect requeue 15s and
     `cond.Message == "Waiting for control plane rollout to start (CPMS generation 2/1 observed)"`
     — the exact message proves routing to the rollout path and that Step 3 is not
     re-run.
   - "stays in worker phase when CPMS not updated": same objects but no ready
     worker/node and `newCPMSUpdatedForRollout([]string{"source-fd-1"}, false)`.
     Expect requeue 30s and
     `cond.Message == "Waiting for target workers (machines 0/0 ready, nodes 0/0 ready)"`
     (readiness is checked before the CPMS gate, so an unready target worker phase
     wins regardless of CPMS state).
   - "reports worker readiness counts while target workers are pending": target MS
     with 2 replicas, one ready machine+node pair plus one Machine in `Provisioning`
     phase. Expect requeue 30s and
     `cond.Message == "Waiting for target workers (machines 1/2 ready, nodes 1/2 ready)"`.
   - "stays in worker phase when target machinesets missing": objects = infra,
     `newCPMSUpdatedForRollout([]string{"target-fd-1"}, false)`,
     `newSourceMachineSetForRollout(2)` (no target MS). Expect requeue 30s,
     `cond.Message == "Workers created, waiting for machines ready"`, and
     `GetMachineSet(targetMSName)` to succeed (the create path ran).
