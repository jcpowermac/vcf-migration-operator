...
Expect PASS. If the envtest binary download fails in this environment (no
network), fall back to the non-envtest packages, which need no envtest
binaries at all:
...
and note the limitation in the report. In environments that already have
envtest assets installed, the full non-e2e run is the stronger fallback:
...
AC3 etcd quorum maintained: COVERED (by delegation to upstream CPMS operator)
  or delegated but unverified, if the versioned rolling-strategy evidence in
  Assumptions could not be confirmed in the target version
  invariant: <grep evidence from Step 4: no CP delete/scale path; etcd CO health gate>
AC4 source CP nodes replaced in controlled sequence: COVERED (by delegation + operator
  sequencing), or delegated but unverified under the same evidence condition
  impl: <anchors from Step 3>
  tests: <names>, all passing
...
- **AC3/AC4 rolling-strategy evidence (versioned):** target release is the
  OpenShift generation built against k8s 1.36 (`go.mod`: `k8s.io/api
  v0.36.0-alpha.0`, `openshift/api v0.0.0-20260511191110-9b69e5fa27e9`). The
  rolling-replacement contract delegated to the upstream CPMS operator is
  grounded in the vendored, versioned machine API: `machinev1.ControlPlaneMachineSet.Spec.Strategy`
  carries `+kubebuilder:default:="RollingUpdate"`
  (`vendor/github.com/openshift/api/machine/v1/types_controlplanemachineset.go:209-215`),
  so the API server applies `RollingUpdate` unless a cluster explicitly sets
  another value.
  `UpdateCPMSFailureDomain` preserves the strategy as-is (it never writes
  `Spec.Strategy`) and now refuses to update a CPMS whose strategy is set to
  anything other than `RollingUpdate` (empty = API-server default), so the
  migration cannot silently stall on an `OnDelete`/`Recreate` CPMS. AC3/AC4
  therefore remain covered-by-delegation with this in-repo evidence; if the
  evidence above cannot be confirmed in the target version, report AC3/AC4 as
  delegated but unverified instead.
