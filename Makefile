...
KIND_CLUSTER ?= vcf-migration-operator-test-e2e
...
test-e2e: setup-test-e2e manifests generate fmt vet ## Run the e2e tests. Expected an isolated environment using Kind.
	RUN_E2E=true KIND_CLUSTER=$(KIND_CLUSTER) go test ./test/e2e/ -v -ginkgo.v
...
.PHONY: docker-build
docker-build: operator-image ## Backward-compatible alias for e2e image builds.
...
# Keep this on a supported envtest release line rather than deriving it from k8s.io/api.
